...
const NotificationItemBase = ({ notification }: { notification: NotificationRecord }) => {
  ...
  return (
    <div className={cn(
      ...
      !notification.isRead && "bg-primary/5 dark:bg-primary/10 hover:bg-primary/10 dark:hover:bg-primary/15"
    )}>
      <div className="flex gap-4 sm:gap-6">
        {/* Icon */}
        <div className={cn(
          "shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center border shadow-sm transition-transform group-hover:scale-105",
          style.bgColor,
          style.borderColor
        )}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h4 className={cn(
              "text-sm font-bold tracking-tight text-foreground group-hover:text-primary transition-colors",
              !notification.isRead && "text-foreground"
            )}>
              {notification.title}
            </h4>
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider shrink-0 mt-0.5">
              {notification.time}
            </span>
          </div>
